...
moves = [[1,2],[1,0],[2,1],[2,2]]
player_A = [moves[i] for i in range (0,len(moves),2)]
player_B = [moves[j] for j in range (1,len(moves),2)]
win_comobo = [[[0,0], [0,1], [0,2]], #top horizontal row
...
if len(moves) ==9:
    win_status = "Draw"
for i in win_comobo:
    if all([a in player_A for a in i]):
        win_status = "A"
        break
    if all([b in player_B for b in i]):
        win_status = "B"
        break
print (win_status)